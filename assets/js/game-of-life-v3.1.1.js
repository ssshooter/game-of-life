...
(function () {
  var weightTL, weightT, weightTR, weightL, weightR, weightBL, weightB, weightBR
  weightTL = weightTR = weightBL = weightBR = 1 // default 1
  weightT = weightB = weightL = weightR = 1 // default 1
  var bornCondition = 3 // default 3
  var deadCondition1 = 2 // default 2
  var deadCondition2 = 3 // default 3
  var randomDeadRate = 0
  var stats = new Stats();
  ...
  var GOL = {
	...
	*/
      nextGeneration: function () {
          ...
          for (j = 1; j < this.actualState[i].length; j++) {
            ...
            for (m = 0; m < 8; m++) {
              if (deadNeighbours[m] !== undefined) {
                // if the neighbour is found in getNeighboursFromAlive, it will asign to undefined
            ...
            if (deadCondition1 <= neighbours && deadCondition2 >= neighbours && Math.random() > randomDeadRate) {
        ...
        // Process dead neighbours
        // dead cell which is surrounded with 3 alive cells will be reborn
        for (key in allDeadNeighbours) {
          if (allDeadNeighbours[key] === bornCondition) { // Add new Cell
            key = key.split(',');
      ...
      /**
       * i actualState line index
       * possibleNeighboursList [[x1,y1,1],[x2,y2,1]]
       */
      getNeighboursFromAlive: function (x, y, i, possibleNeighboursList) {
        var neighbours = 0, k;
        // use pointer to skip former state
        // Top
